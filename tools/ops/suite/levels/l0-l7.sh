...
run bash -lc "cd \"$ROOT\" && ./tools/gen-vault-abi"
run bash -lc "cd \"$ROOT\" && ./tools/check-generated.sh"
...
run bash -lc "cd \"$ROOT\" && ./tools/verify/law-kernel.sh"

step "L2 - Core Verify (TLA + build + compliance baseline)"
run bash -lc "cd \"$ROOT\" && ./tools/verify/core.sh"

step "L3 - Workspace Lifecycle Gate"
run bash -lc "cd \"$ROOT\" && ./tools/gates/ws.sh \"${WS_PREFIX}_ws\""

step "L4 - Cortex Determinism Gate"
run bash -lc "cd \"$ROOT\" && ./tools/gates/cortex.sh \"${WS_PREFIX}_cortex\""

step "L5 - Event Stream Reliability Gate"
run bash -lc "cd \"$ROOT\" && ./tools/gates/events.sh"

step "L6 - Graph Gate"
run bash -lc "cd \"$ROOT\" && ./tools/gates/graph.sh \"${WS_PREFIX}_graph\""



step "L7 - Providers + Rust Unit/Integration Tests + CLI Smoke"
PROVIDERS_WS="${WS_PREFIX}_prv_$RANDOM"
run bash -lc "cd \"$ROOT\" && ./tools/gates/providers.sh \"${PROVIDERS_WS}\""
...
if [[ "$DATASET_GATE" == "1" ]]; then
  ...
  run bash -lc "cd \"$ROOT\" && BIN=\"$YAI_BIN\" ./tools/gates/dataset-global-stress.sh \"${WS_PREFIX}_dataset\""
fi

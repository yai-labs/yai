...
WS="ws_graph_read_surfaces_dp16_v1"
...
(cd "$REPO" && env -u YAI_RUNTIME_INGRESS "$YAI" >/tmp/yai_workspace_graph_read_surfaces_dp16.log 2>&1) &
RUNTIME_PID=$!
...
[[ -S "$SOCK" ]] || { echo "workspace_graph_read_surfaces_dp16_v1: FAIL (missing ingress socket)"; exit 1; }
...
checks = [
    ...
    ("yai.workspace.graph.workspace", "graph.workspace", "graph_neighborhood_table"),
    ("yai.workspace.graph.governance", "graph.governance", "summary_card"),
    ("yai.workspace.graph.decision", "graph.decision", "detail_record"),
    ("yai.workspace.graph.artifact", "graph.artifact", "summary_card"),
    ("yai.workspace.graph.authority", "graph.authority", "summary_card"),
    ("yai.workspace.graph.evidence", "graph.evidence", "summary_card"),
    ("yai.workspace.graph.lineage", "graph.lineage", "timeline"),
    ("yai.workspace.graph.recent", "graph.recent", "table"),
]
...
for cmd, family, shape in checks:
    ...
    rp = data.get("read_path", {})
    assert rp.get("mode") == "db_first", (cmd, rp)
    assert rp.get("filesystem_primary") is False, (cmd, rp)

q = call("system", "yai.workspace.query", ["graph.workspace"])
assert q["status"] == "ok", q
assert q["data"]["query_family"] == "graph.workspace", q

r = call("system", "yai.workspace.unset")
assert r["status"] == "ok", r
PY

echo "workspace_graph_read_surfaces_dp16_v1: ok"

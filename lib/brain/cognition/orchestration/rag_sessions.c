...
static yai_mind_rag_session_state_t g_sessions[YAI_MIND_RAG_SESSION_CAP];
                                 ...
                                 yai_mind_rag_session_state_t **session_out)
{
  ...
  if (slot < 0) {
    ...
    if (yai_mind_arena_init(&g_sessions[slot].arena, YAI_MIND_RAG_ARENA_SIZE) != YAI_MIND_OK) {
      return YAI_MIND_ERR_NO_MEMORY;
    }
    g_sessions[slot].initialized = 1;
  } else {
    yai_mind_arena_reset(&g_sessions[slot].arena);
  }
...
void yai_mind_rag_sessions_reset(void)
{
  for (int i = 0; i < YAI_MIND_RAG_SESSION_CAP; i++) {
    if (g_sessions[i].initialized) {
      yai_mind_arena_destroy(&g_sessions[i].arena);
    }

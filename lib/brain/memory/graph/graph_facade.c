...
{
  if (!yai_mind_graph_backend_ops()) return yai_mind_graph_backend_select_inmemory();
  return YAI_MIND_OK;
}
                               ...
                               yai_mind_node_id_t *node_id_out)
{
  ...
  return yai_mind_graph_backend_ops()->node_create(domain, key, value, node_id_out);
}
                               ...
                               yai_mind_edge_id_t *edge_id_out)
{
  int rc = ensure_backend();
  if (rc != YAI_MIND_OK) return rc;
  return yai_mind_graph_backend_ops()->edge_create(from_node, to_node, relation, weight, edge_id_out);
}
...
int yai_mind_graph_node_get(yai_mind_node_id_t node_id, yai_mind_graph_node_t *node_out)
{
  int rc = ensure_backend();
  if (rc != YAI_MIND_OK) return rc;
  return yai_mind_graph_backend_ops()->node_get(node_id, node_out);
}
...
int yai_mind_graph_edge_get(yai_mind_edge_id_t edge_id, yai_mind_graph_edge_t *edge_out)
{
  int rc = ensure_backend();
  if (rc != YAI_MIND_OK) return rc;
  return yai_mind_graph_backend_ops()->edge_get(edge_id, edge_out);
}
...
int yai_mind_graph_stats_get(yai_mind_graph_stats_t *stats_out)
{
  int rc = ensure_backend();
  if (rc != YAI_MIND_OK) return rc;
  return yai_mind_graph_backend_ops()->stats(stats_out);
}
                              ...
                              yai_mind_memory_result_t *result_out)
{
  int rc = ensure_backend();
  if (rc != YAI_MIND_OK) return rc;
  return yai_mind_graph_backend_ops()->query(query, result_out);
}
...
{
  return yai_mind_graph_backend_select_rpc(endpoint);
}

const char *yai_mind_graph_backend_name(void)
{
  return yai_mind_graph_backend_name_internal();
}

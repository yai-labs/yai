...
static yai_mind_semantic_record_t g_semantic[YAI_MIND_SEMANTIC_CAP];
                                 ...
                                 yai_mind_node_id_t node_id)
{
  ...
  for (i = 0; i < g_semantic_count; i++) {
    if (strcmp(g_semantic[i].term, term) == 0) {
      ...
      if (yai_mind_node_id_is_valid(node_id)) g_semantic[i].node_id = node_id;
      return YAI_MIND_OK;
    }
  }
  ...
  if (!yai_mind_node_id_is_valid(node_id)) {
    int rc = yai_mind_graph_node_create("semantic", term, definition, &node_id);
...
void yai_mind_graph_semantic_reset(void)
{

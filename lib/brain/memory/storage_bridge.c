                                   ...
                                   size_t transient_index_cap)
{
  ...
  if (snprintf(base, sizeof(base), "%s/.yai/run/%s/brain", home, workspace_id) <= 0) return -1;
                                            ...
                                            size_t err_cap)
{
  ...
  (void)snprintf(graph_node_ref, sizeof(graph_node_ref), "%s", node_ref_decision);
  (void)snprintf(graph_edge_ref, sizeof(graph_edge_ref), "%s", edge_ref_evidence_for_decision);
  ...
  (void)yai_mind_graph_node_create("workspace", workspace_id, "runtime_scope", &workspace_node);
  (void)yai_mind_graph_node_create("governance", governance_slug, governance_ref_local[0] ? governance_ref_local : "none", &governance_node);
  (void)yai_mind_graph_node_create("decision", decision_slug, effect && effect[0] ? effect : "unknown", &decision_node);
  (void)yai_mind_graph_node_create("evidence", evidence_slug, evidence_ref && evidence_ref[0] ? evidence_ref : "none", &evidence_node);
  (void)yai_mind_graph_node_create("authority", authority_slug, authority_profile && authority_profile[0] ? authority_profile : "unknown", &authority_node);
  (void)yai_mind_graph_node_create("artifact", artifact_slug, resource_hint && resource_hint[0] ? resource_hint : "unknown", &artifact_node);
  (void)yai_mind_graph_node_create("episode", event_slug, family_slug, &episode_node);

  (void)yai_mind_graph_edge_create(decision_node, workspace_node, "decision_in_workspace", 1.0f, &edge_decision_in_workspace);
  (void)yai_mind_graph_edge_create(decision_node, governance_node, "decision_under_governance", 1.0f, &edge_decision_under_governance);
  (void)yai_mind_graph_edge_create(decision_node, authority_node, "decision_under_authority", 1.0f, &edge_decision_under_authority);
  (void)yai_mind_graph_edge_create(decision_node, artifact_node, "decision_on_artifact", 1.0f, &edge_decision_on_artifact);
  (void)yai_mind_graph_edge_create(evidence_node, decision_node, "evidence_for_decision", 1.0f, &edge_evidence_for_decision);
  (void)yai_mind_graph_edge_create(artifact_node, governance_node, "artifact_governed_by", 1.0f, &edge_artifact_governed_by);
  (void)yai_mind_graph_edge_create(workspace_node, governance_node, "workspace_uses_governance", 1.0f, &edge_workspace_uses_governance);
  (void)yai_mind_graph_edge_create(episode_node, decision_node, "episode_yielded_decision", 1.0f, &edge_episode_yielded_decision);
  ...
  #define APPEND_NODE_ROW(REF, CLS, NID, ORIGIN_DOMAIN, SOURCE_REF) \
    do { \
      if (snprintf(node_row, sizeof(node_row), \
                   "{\"type\":\"yai.brain_graph_node.v1\",\"schema_version\":\"v1\"," \
  ...
  #define APPEND_EDGE_ROW(REF, CLS, EID, FROM, TO) \
    do { \
      if (snprintf(edge_row, sizeof(edge_row), \
                   "{\"type\":\"yai.brain_graph_edge.v1\",\"schema_version\":\"v1\"," \
  ...
  if (snprintf(graph_index_row,
               ...
               "{\"type\":\"yai.brain.graph.index.v1\",\"workspace_id\":\"%s\","
  ...
  if (snprintf(transient_index_row,
               ...
               "{\"type\":\"yai.brain.transient.index.v1\",\"workspace_id\":\"%s\","

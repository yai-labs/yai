...
static void map_family_to_domain_id(const char *family_id, char *out, size_t out_cap) {
  ...
  domain_id[0] = '\0';
  if (yai_law_domain_model_family_resolution(family_id,
                                             domain_id,
                                             sizeof(domain_id),
                                             NULL,
                                             0,
                                             ...
                                             NULL) == 0 &&
      domain_id[0] != '\0') {
    (void)yai_law_safe_snprintf(out, out_cap, "%s", domain_id);
    return;
  }
...
static int specialization_exists(const char *family_id, const char *specialization_id) {
  ...
  return yai_law_domain_model_specialization_exists(family_id, specialization_id);
}
...
static void choose_specialization(const yai_law_classification_ctx_t *ctx,
  ...
  (void)yai_law_domain_model_family_resolution(family_id,
                                               NULL,
                                               0,
  ...
  if (strcmp(family_id, "economic") == 0) {
    ...
    else if (default_specialization[0]) (void)yai_law_safe_snprintf(out->specialization_id, sizeof(out->specialization_id), "%s", default_specialization);
    ...
    if (ctx->black_box_mode || strstr(ctx->command, "black-box") || strstr(ctx->command, "black_box")) {
      ...
      (void)yai_law_safe_snprintf(out->specialization_id, sizeof(out->specialization_id), "%s", default_specialization);
    } else {
      (void)yai_law_safe_snprintf(out->specialization_id, sizeof(out->specialization_id), "%s", "parameter-governance");
    }
  } else {
  ...
  if (!specialization_exists(family_id, out->specialization_id)) {
    if (default_specialization[0]) {
      ...
      (void)yai_law_safe_snprintf(out->specialization_id, sizeof(out->specialization_id), "%s", candidates[0]);
    } else if (strcmp(family_id, "economic") == 0) {
      (void)yai_law_safe_snprintf(out->specialization_id, sizeof(out->specialization_id), "%s", "payments");
    } else if (strcmp(family_id, "scientific") == 0) {
      (void)yai_law_safe_snprintf(out->specialization_id, sizeof(out->specialization_id), "%s", "parameter-governance");
    } else {
      (void)yai_law_safe_snprintf(out->specialization_id, sizeof(out->specialization_id), "%s", "network-egress");
...
int yai_law_discover_domain(const yai_law_classification_ctx_t *ctx,
  ...
  if (yai_law_domain_model_runtime_families(families, 4, &family_count) != 0 || family_count <= 0) {
    (void)yai_law_safe_snprintf(families[0], sizeof(families[0]), "%s", "digital");
    (void)yai_law_safe_snprintf(families[1], sizeof(families[1]), "%s", "economic");
    (void)yai_law_safe_snprintf(families[2], sizeof(families[2]), "%s", "scientific");
    family_count = 3;
  }

...
{
  puts("yai-daemond - canonical daemon plane manager");
  printf("version: %s\n", YAI_VERSION_STRING);
  puts("");
  puts("usage: yai-daemond [options]");
  puts("  --help                    show help");
  ...
  puts("");
  puts("guardrails:");
  puts("  - yai-daemond is subordinate to owner workspace sovereignty");
  ...
  puts("  - no final policy/authority/policy/graph/conflict truth locally");
...
static int parse_args(int argc, char **argv, yai_edge_config_t *cfg)
{
  ...
  for (i = 1; i < argc; ++i)
  {
    ...
    fprintf(stderr, "yai-daemond: unknown/invalid argument: %s\n", argv[i]);
    return -1;
  }
...
int main(int argc, char **argv)
{
  ...
  {
    fprintf(stderr, "yai-daemond: failed to initialize defaults\n");
    return 70;
  }
  ...
  if (rc != 0)
  {
    fprintf(stderr, "yai-daemond: config validation failed rc=%d\n", rc);
    return 78;
  }
  ...
  {
    fprintf(stderr,
            "yai-daemond: background mode placeholder only; use --mode foreground in YD-2\n");
    return 78;
  }
  ...
  if (rc != 0)
  {
    fprintf(stderr, "yai-daemond: runtime init failed rc=%d\n", rc);
    return 70;
  }
  if (yai_edge_lifecycle_install_signals() != 0)
  {
    fprintf(stderr, "yai-daemond: failed to install signal handlers\n");
    return 70;
  }
  if (yai_edge_runtime_start(&rt) != 0)
  {
    fprintf(stderr, "yai-daemond: runtime start failed\n");
    return 70;
  }
  ...
  if (rc != 0)
  {
    fprintf(stderr, "yai-daemond: foreground loop failed rc=%d\n", rc);
  }

  if (yai_edge_runtime_shutdown(&rt) != 0)
  {
    fprintf(stderr, "yai-daemond: shutdown failed\n");
    return 70;
  }

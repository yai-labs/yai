...
python3 "$ROOT/tools/validate/validate_overlay_compliance_view.py"
...
for t in \
 ...
 do
  cc $CFLAGS $INCLUDES "$ROOT/tests/legacy/unit/governance/${t}.c" $GOVERNANCE_SRCS -o "$OUT_DIR/$t"
  "$OUT_DIR/$t"
 done

python3 "$ROOT/tests/legacy/unit/governance/test_governance_manifest_surface_contract.py"

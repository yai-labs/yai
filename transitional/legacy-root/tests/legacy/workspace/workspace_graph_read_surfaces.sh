...
(cd "$REPO" && env -u YAI_RUNTIME_INGRESS "$YAI" >/tmp/yai_workspace_graph_read_surfaces.log 2>&1) &
RUNTIME_PID=$!
...
[[ -S "$SOCK" ]] || { echo "workspace_graph_read_surfaces: FAIL (missing ingress socket)"; exit 1; }
...
echo "workspace_graph_read_surfaces: ok"

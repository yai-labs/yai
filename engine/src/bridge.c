...
static Vault* attach_shm(const char* shm_path) {
    ...
    if ((size_t)st.st_size < sizeof(Vault)) {
        close(fd);
        return NULL;
    }

    Vault* v = (Vault*)mmap(NULL, sizeof(Vault), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    close(fd);
...
int yai_bridge_init(const char* ws_id) {
    ...
    _vault = (Vault*)mmap(NULL, sizeof(Vault), PROT_READ | PROT_WRITE, MAP_SHARED, _shm_fd, 0);
...
Vault* yai_get_vault(void) { return _vault; }

...
| `cli` | operator surface consumer | no structural cross-repo pin | pinning `governance`, pinning `sdk` | non-authoritative for governance/sdk | declares supported compatibility | may run verify-only compatibility checks | verify hooks are not repo dependency |

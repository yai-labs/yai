...
Define mandatory citation and alignment rules between `yai` architecture docs and `deps/yai-law` normative sources.

## Rules

- Every architecture document must include `law_refs` in frontmatter.
- Architecture statements that mention protocol/roles/errors must link a concrete `deps/yai-law/...` path.
...
If architecture claims require cross-repo behavior changes (`yai`, `yai-cli`, `yai-law`), track as coordinated updates through runbook/MP references.

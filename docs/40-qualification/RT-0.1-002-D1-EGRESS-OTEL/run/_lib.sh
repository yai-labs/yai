...
WORKLOAD_ID="${WORKLOAD_ID:-wrk-d1-egress-otel-v1}"
ATTACK_PROFILE_ID="${ATTACK_PROFILE_ID:-rt-002-otel-export-egress-attempt}"
...
WS_ID="${WS_ID:-ws-rt002-${RUN_ID}}"
TRACE_ID="${TRACE_ID:-trace-rt002-${RUN_ID}}"
RT_ID="${RT_ID:-rt002}"
...
TARGET_HOST="${TARGET_HOST:-$([[ "$TARGET_PROFILE" == "remote" ]] && echo otel.${REMOTE_DOMAIN} || echo 127.0.0.1)}"
TARGET_PORT="${TARGET_PORT:-$([[ "$TARGET_PROFILE" == "remote" ]] && echo 443 || echo 8443)}"
TARGET_PATH="${TARGET_PATH:-/v1/traces}"

...
## 0) Intent
This runbook establishes a deterministic, enterprise-grade foundation for **`governance`** and its consumers (**`yai`**, **`cli`**) *before* runtime hardening tracks (Root Hardening, Workspace Lifecycle, Engine Attach, Data Plane).
...
- [x] No direct development is performed inside consumer vendored specs trees (e.g. `yai/../governance`).
- [x] Any change in `governance` that affects consumers is coordinated via pins/tags and verified in consumers.

---

## 3) Inputs / Repos
### Repos involved
- `governance` (canonical specs)
- `yai` (consumer/runtime)
- `cli` (consumer)

### Canonical references (in `governance`)
...
- `https://github.com/yai-labs/governance/issues/9`
...
### Repo-of-truth rule
- `governance` is **source of truth** for specs/contracts/formal/vectors.
- `yai` and `cli` **consume pinned versions** (tag/commit) of `governance`.
...
**MP (planned):**
- `docs/archive/legacy/program/milestone-packs/specs-refactor-foundation/mp-governance-000-specs-refactor-foundation-v0-1-8.md`
...
**Claim:** `yai` consumes `governance` deterministically under the new structure.  
...
**Scope:** `governance` CI and repo gates.
...
### In `governance` (foundation gates)
```bash
make ci
make validate
make format-check
...
- Roll back to last known good `governance` tag/commit in consumers.
...
- Specs: `../governance/SPEC_MAP.md`, `../governance/REGISTRY.md`
...
- `tools/bin/yai-verify governance-kernel` -> PASS

...
related:
  ...
  - ../governance/docs/program/milestone-packs/runtime-baselines/governance-topology-realign-against-unified-runtime.md
...
- LW outcomes in `yai-governance` (runtime targeting, registry/primitives/governable objects, machine-readable entrypoints/constraints)
...
- Structural checks pass (`make check`, `make validate-governance-registry`).
...
- Machine-readable governance/runtime contracts: converged with compatibility debt (`yai-governance` disallows brain/mind as canonical but still carries alias vectors).
...
| --- | --- | --- | --- | --- |
| `yai-governance` | `registry/commands.v1.json` legacy aliases | transitional compatibility | Legacy alias vectors retained for command compatibility. | no |
| `yai-governance` | selected schema/manifests with `brain`/`mind` compatibility enums | transitional compatibility | Retained as explicit non-canonical compatibility fields. | no |
| `yai-governance` | `runtime/brain/*` | historical only | De-authorized historical subtree retained for traceability. | no |
...
Basis:
- Runtime topology, governance targeting, SDK contracts, and CLI operator surfaces are converged on one model.
- Core SDK and CLI test suites are green.
- Governance structural and registry validation gates are green.

Non-blocking caveat:
- Transitional compatibility aliases remain in `yai-governance`/`yai-cli` outside canonical active paths.
...
- retire remaining compatibility aliases in `yai-governance` and `yai-cli`.

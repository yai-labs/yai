...
use crate::memory::graph::domains::activation::trace::ActivationTrace;
...
impl ActivationTraceStore {
    pub fn open(layout: WorkspaceLayout) -> Result<Self> {
        let db_path = layout.root.join("activation.sqlite");
    ...
    pub fn record_run(
        ...
        if meta.ws_id != self.layout.ws_id {
    ...
    pub fn get_run(&self, run_id: &str) -> Result<Option<ActivationTrace>> {
        ...
        if meta.ws_id != self.layout.ws_id {
            bail!("activation trace ws mismatch");
        }
    ...
    pub fn list_runs(&self, limit: usize, offset: usize) -> Result<Vec<(String, i64, String)>> {
        ...
        let rows = stmt.query_map(
            params![self.layout.ws_id, limit as i64, offset as i64],
...
fn build_trace(
    ...
    let hits = results
        .into_iter()
        .map(
            |row| crate::memory::graph::domains::activation::api::ActivationHit {
                node: row.node_id,
                score_q: row.score_q,
